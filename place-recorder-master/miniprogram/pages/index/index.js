...
var page = {
    ...
    wx.getUserProfile({
      ...
      success: (e) => {
        ...
        if (e.userInfo) {
          ...
          console.log("初次授权 e.detail.userInfo: ", e.userInfo);
          this.setData({
            userInfo: e.userInfo,
            hasUserInfo: true
          });
          ...
          // 冗余调用一次 wx.getUserProfile()，确保云开发控制台—>运营分析->用户访问列表中出现用户详细信息。
          wx.getUserProfile({
            success: res => {
              console.log("初次授权 wx.getUserProfile: ", res.userInfo)
            }
          });
  ...
  goUploadform: function () {
		wx.navigateTo({
			url: '/pages/uploadform/uploadform',
  })
  },
  goCamera: function () {
		wx.navigateTo({
