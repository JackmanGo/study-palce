...
Page({
	...
	getTime: function () {
		timer = setInterval(() => {
			let timeData = Utils.formatTimeMany(new Date)
			this.setData({
				date: timeData.date,
				time: timeData.time,
				week: timeData.week
	...
	takePhoto: function () {
        ...
        const prevPage = pages[pages.length - 2]; // 获取表单页面实例，-1是相机拍照页面
        
        if (prevPage) {
          // 直接调用表单页面的方法更新图片
          prevPage.setCameraImage(addWatermark);
        } 
        // 返回上一页
        wx.navigateBack();
			}
